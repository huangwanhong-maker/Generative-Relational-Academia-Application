...
import { runtimePaths, grrpCommand } from '../packages/server/src/runtime.js'

const BASE = process.env.GRA_BASE ?? 'http://127.0.0.1:8001'
if (!process.env.GRA_RUNTIME || !process.env.GRA_BASE) {
  throw new Error('Demo seeding requires explicit GRA_RUNTIME and GRA_BASE for a separate test server.')
}
const paths = runtimePaths()
const command = grrpCommand()
const SLUG = 'field-memory'
const DIR = join(paths.recordsRoot, SLUG)
...
async function main() {
  ...
  execFileSync(process.env.GRA_PYTHON ?? command[0]!, ['-c',
